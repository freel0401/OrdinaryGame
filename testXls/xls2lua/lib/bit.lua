...
function _G.setBitMap(map, bits, unset) -- unset true 为上锁
	local min, max = 1, #map * 53
	local bitfunc
	if unset==true then  bitfunc=bit.clsbita else bitfunc=bit.setbita end
	if #bits >0 then
		local len=0
		for i,bit in next, bits do
			if bit>max then assert(nil,'setBitMap: bit '..bit..' is bigger than max number '..max) end
			if bit<min then assert(nil,'setBitMap: bit '..bit..' is less than min number 1'..min) end
			map =  bitfunc(map,bit)
			len=len+1
		end
		assert(len==#bits,'setBitMap: bits should be an array')
	end
	return map
end

function _G.checkBitMap(map, cbit, unset)
	local min, max = 1, #map * 53
	if cbit<min or cbit>max then
		error('checkBitMap: bit='..cbit..' is out of map range '..min..' - '..max)
	end
	return unset and (not bit.chkbita(map,cbit)) or bit.chkbita(map,cbit)
end